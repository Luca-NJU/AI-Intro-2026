...
def cornersHeuristic(state, problem):
    ...
    h_sum = 0
    "*** YOUR CODE HERE ***"
    if isinstance(Visited_Corners, list) or isinstance(Visited_Corners, tuple):
        arrived_set = set(Visited_Corners)
    else:
        arrived_set = set(Visited_Corners) if not isinstance(Visited_Corners, set) else cornerArrived
    unvisited = [corner for corner in corners if corner not in Visited_Corners]
    
    if not unvisited:
        return 0
    
    distances = [(util.manhattanDistance(node, c), c) for c in unvisited]
    dist, closest = min(distances)

    new_cornerArrived = frozenset(arrived_set | {closest})
    h_sum = dist + cornersHeuristic((closest, new_cornerArrived), problem)
